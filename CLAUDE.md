# SpektraLab — read `AGENTS.md` first

This repository is the macOS product **SpektraLab** (renamed from Filmify,
2026-09-16): a SwiftUI app with a C++ Metal render engine compiled into it.
`README.md` is the map; `AGENTS.md` is the working notes, and the traps
section in it has cost real debugging time.
...
- **The engine is spektrafilm's.** The product is SpektraLab; the engine, the 28
...
- **The Xcode target and scheme are still called `Spektrafilm`** while the
  product is SpektraLab. That is intentional (schemes reference the target by
