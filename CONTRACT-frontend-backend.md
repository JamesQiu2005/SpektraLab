...
1. **`rgba16` layout.** `width × height × 4 × uint16`, row-major, row 0 is the
   top of the image, values already encoded **in the session's output colour
   space**. FE applies no transfer curve and never will (`frontend_architecture.md`
   §4, "Colour, stated once"). A GPU-native BE that lands linear values, or
   BGRA, or bottom-up, produces a washed-out or upside-down canvas and no test
   on either side will say so.

   > **Which space that is — RFC-018 §5.1.** A session opened by `spk_open`
   > renders to **ProPhoto RGB, CCTF encoded**: the *working space*, in which
   > the frontend grades, and not a display space. `io.output_color_space` in
   > an open delta may override it; a caller that overrides it owns the
   > consequences, because the frontend's output transform reads the session's
   > *resolved* space rather than a constant. **The resolved space is echoed in
   > the `open` reply's `params.io` and MUST be read from there.** It used to
   > be Display P3 — right on screen, hardcoded in `spk_open`, and stated
   > nowhere outside a comment in the C header (RFC-018 §4.2).
   >
   > Leaving the working space is the frontend's job, once, per destination:
   > the canvas converts to Display P3, and an export converts to the recipe's
   > space. The numbers it converts with come from the engine over
   > `spk_output_transform` (RFC-018 §5.2) — a C ABI call, not a wire method.
   > Nothing about it crosses the transport, and no method, field or tier name
   > changed.
...
| 2026-09-13 | transport 1, schema 1 | **Behaviour, not wire (RFC-018 §5.1).** A session's output colour space is now **ProPhoto RGB, CCTF encoded** — the working space the app grades in — instead of the Display P3 that `spk_open` used to hardcode. `io.output_color_space` in an open delta still overrides it, and the resolved value is echoed in the open reply's `params.io`; the frontend reads it from there and converts out of it with the new `spk_output_transform` C ABI call (not a wire method). **No method, field, tier name, version or `rgba16` layout changed** — the bytes are still little-endian rgba16, row 0 = top; what changed is *which space* those encoded values are in. `params.hpp`'s declared default read `"sRGB"` while the behaviour was Display P3, and the two now agree. | A (RFC-018 stream 1) |
