...
struct BrowseCell: View {
    ...
    ///
    /// **Not** how the editor marks a frame — see `framing`. This one is the
    /// export page's, which draws every chosen frame the same way, at the
    /// weight `notes.md` gives it, whether or not one of them is also on the
    /// canvas.
    var chosen: Bool? = nil
